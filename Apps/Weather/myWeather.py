...
# Step 1: Fetch temperature, weather code, wind speed, and daily high/low temperatures
def get_weather_data():
    ...
    params = {
        ...
        "daily": ["temperature_2m_max", "temperature_2m_min"],  # Request daily high and low
        "timezone": "auto"  # Ensure the correct timezone
    }
    ...
    try:
        ...
        # Fetch the current weather (temperature, weather code, wind speed)
        current_weather = data.get('current_weather', {})
        current_temperature = current_weather.get('temperature', None)  # Fetch current temperature
        weather_code = current_weather.get('weathercode', None)  # Fetch weather code
        wind_speed = current_weather.get('windspeed', None)  # Fetch wind speed
        
        # Fetch the daily high and low temperatures
        daily = data.get('daily', {})
        daily_high = daily.get('temperature_2m_max', [None])[0]  # Get today's high temperature
        daily_low = daily.get('temperature_2m_min', [None])[0]  # Get today's low temperature
        
        if None in (current_temperature, weather_code, wind_speed, daily_high, daily_low):
            print("Error: Could not retrieve all weather data.")
            return None, None, None, None, None
        
        return current_temperature, weather_code, wind_speed, daily_high, daily_low
...
# Step 3: Scroll temperature, weather description, wind speed, high/low temps across LED matrix
...
def main():
    ...
    temperature, weather_code, wind_speed, daily_high, daily_low = get_weather_data()  # Initial fetch of weather data
    ...
    while True:  # Infinite loop to display and refresh
        ...
        if t.time() - last_update_time >= update_interval:
            temperature, weather_code, wind_speed, daily_high, daily_low = get_weather_data()  # Fetch new weather data
            last_update_time = t.time()  # Reset the last update time
        
        # Step 2: If valid temperature, weather, wind speed, and daily high/low, interpret the weather code
        if temperature is not None and weather_code is not None and wind_speed is not None:
            ...
            message = f"Temp: {temperature:.1f}C, High: {daily_high:.1f}C, Low: {daily_low:.1f}C, Wind: {wind_speed:.1f}km/h, {weather_description}"  # Create message
