...
MATRIX_WIDTH  = 3
MATRIX_HEIGHT = 4
ROTATE_BLOCKS_BY_180_DEGREES = True
...
gfx_buffer  = [[0 for x1 in range(MATRIX_HEIGHT*8)] for x2 in range(MATRIX_WIDTH*8)]
...
#BY Peter Didow September 12 2024				
def gfx_letter_rotated_90(char_code, start_x=0, start_y=0, state=GFX_INVERT, font=DEFAULT_FONT):
    # This version of gfx_letter rotates the character by 90 degrees and writes it left to right
    start_x = int(start_x)
    start_y = int(start_y)
    for l_col in range(0, 8):  # We switch rows and columns to rotate the character
        for l_row in range(0, 8):
            if (l_row + start_x) in gfx_columns and (7 - l_col + start_y) in gfx_rows:  # Rotated coordinates
                bit = (font[char_code][l_col] & (1 << l_row)) >> l_row  # Swapped bits for 90-degree rotation
                if state == GFX_ON:
                    gfx_buffer[l_row + start_x][7 - l_col + start_y] = bit
                elif state == GFX_OFF:
                    gfx_buffer[l_row + start_x][7 - l_col + start_y] = ~bit
                elif state == GFX_INVERT:
                    gfx_buffer[l_row + start_x][7 - l_col + start_y] = bit ^ gfx_buffer[l_row + start_x][7 - l_col + start_y]

def gfx_letter_rotated_90_clockwise(char_code, start_x=0, start_y=0, state=GFX_INVERT, font=DEFAULT_FONT):
    # This version of gfx_letter rotates the character by 90 degrees and writes it left to right
    start_x = int(start_x)
    start_y = int(start_y)
    
    for l_row in range(8):  # Loop through rows of the original character
        for l_col in range(8):  # Loop through columns of the original character
            if (l_row + start_x) in gfx_columns and (l_col + start_y) in gfx_rows:  # Check if within display bounds
                bit = (font[char_code][l_row] & (1 << (7 - l_col))) >> (7 - l_col)  # Extract bit for rotation
                if state == GFX_ON:
                    gfx_buffer[l_col + start_x][7 - l_row + start_y] = bit  # Rotate and write bit
                elif state == GFX_OFF:
                    gfx_buffer[l_col + start_x][7 - l_row + start_y] = ~bit  # Invert and write bit
                elif state == GFX_INVERT:
                    gfx_buffer[l_col + start_x][7 - l_row + start_y] = bit ^ gfx_buffer[l_col + start_x][7 - l_row + start_y]  # Invert existing bit
