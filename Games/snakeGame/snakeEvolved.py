...
def is_valid_target_position(pos):
    return pos not in tail and pos not in walls


def is_valid_wall_position(pos):
    return pos not in tail and pos != tail[0] and pos not in walls
...
def createWalls():
    ...
    walls = []
    for _ in range(10):
        wall_size = randrange(3, 7)
        start_point = Point(randrange(1, WIDTH - wall_size), randrange(1, HEIGHT - wall_size))
        walls.extend([start_point + Point(i, j) for i in range(wall_size) for j in range(wall_size) if is_valid_wall_position(start_point + Point(i, j))])
