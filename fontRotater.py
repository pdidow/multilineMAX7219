...
# Function to rotate a byte (character) 90 degrees
def rotate_byte(byte):
    rotated_byte = 0
    for i in range(8):
        bit = (byte >> i) & 0x01
        rotated_byte |= bit << (7 - i)
    return rotated_byte

# Create a 90-degree rotated copy of the TINY_FONT
ROTATED_TINY_FONT = []
for row in zip(*TINY_FONT[::-1]):
    rotated_row = [rotate_byte(byte) for byte in row]
    ROTATED_TINY_FONT.append(rotated_row)

# Print the rotated font for reference
for row in ROTATED_TINY_FONT:
    hex_row = ["0x{:02X}".format(byte) for byte in row]
    print(hex_row)